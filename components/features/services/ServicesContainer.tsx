...
export default function ServicesContainer({
  ...
  const handleCreateService = async (data: ServiceFormValues) => {
    const createPromise = createService(data).then((newService) => {
  ...
  const handleEditService = async (id: string, data: ServiceFormValues) => {
    const editPromise = updateService(data, id).then((updatedService) => {
      setServices((prev) =>
        prev.map((service) =>
  ...
  const handleDeleteService = async (id: string): Promise<void> => {
    const deletePromise = deleteService(id).then(() => {
    ...
    <div className="w-full flex flex-col items-start justify-between gap-4">
      <div className="w-full flex items-center justify-between gap-4">
        ...
        <Button
          ...
          variant="primary"
          className="w-full md:w-auto !text-sm font-normal whitespace-nowrap"
        >
          Crear servicio +
        </Button>
      </div>
